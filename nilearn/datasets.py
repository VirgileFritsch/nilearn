...
def fetch_oasis_vbm(n_subjects=None, dartel_version=True,
    ...
    with the following steps:
      ...
      6. Resampling to reduce shape and make it correspond to the shape of
         the non-DARTEL data (fetched with dartel_version=False).
      7. Replacement of values < 1e-4 with zeros to reduce the file size.
    ...
    for the 416 available subjects is provided. Externals variates (age,
    gender, estimated intracranial volume, years of education,
    socioeconomic status, dementia score) are provided in a CSV file that is
    a copy of the original Oasis CSV file. The current downloader loads the
    CSV file and keeps only the lines corresponding to the subjects actually
    demanded.
    ...
    if dartel_version:  # DARTEL version has 12 identified outliers
        if n_subjects is None or n_subjects > 404:
            n_subjects = 404
    else:  # all subjects are available with non-DARTEL version
        if n_subjects is None or n_subjects > 416:
            n_subjects = 416
    if n_subjects < 1:
    ...
    if url is None:
        if dartel_version:
            url_images = ('https://www.nitrc.org/frs/download.php/'
                          '6358/archive_dartel.tgz?i_agree=1&download_now=1')
        else:
            url_images = ('https://www.nitrc.org/frs/download.php/'
    ...
    if dartel_version:
        ...
        removed_outliers = [24, 52, 61, 89, 122, 157, 222, 269, 282, 287, 309,
                            428]
        missing_subjects = sorted(missing_subjects + removed_outliers)
        file_names_gm = [
            (os.path.join(
                    "OAS1_%04d_MR1",
                    "wrc1OAS1_%04d_MR1_mpr_anon_fslswapdim_bet.nii.gz")
             % (s, s),
             url_images, opts)
        ...
        file_names_wm = [
            (os.path.join(
                    "OAS1_%04d_MR1",
                    "wrc2OAS1_%04d_MR1_mpr_anon_fslswapdim_bet.nii.gz")
             % (s, s),
             url_images, opts)
    ...
    # Keep CSV information for selected subjects
